...
(function() {
    ...
    const kinopoisk = function() {
        $(document).ready(function() {
            ...
            GM.setValue('NameFilm', $NameFilm); GM.setValue('Alternative_NameFilm', $('span.styles_originalTitle__JaNKM').text()); GM.setValue('LinkFilm', window.location.pathname); GM.setValue('URL_Poster', $('img.film-poster').attr('src'));
    ...
    const watching = function() {
        ...
        console.log($Video_pleer);

        GM.getValue('LinkFilm').then(pathname => {
            if (pathname !== window.location.pathname) {
                $('title').text(`Кинопоиск.`);
                $('<i>', {style: 'margin-top: 15px; margin-bottom: 15px; margin-left: 15px', class: 'back-arrow'}).click(function() {window.location.host = 'www.kinopoisk.ru'}).insertBefore($Video_pleer);
                $Video_pleer.css('justify-content', 'center').find('div:eq(0)').css('width', '50%');
            } else {
                GM.getValue('NameFilm').then(name_of_film => {
                    let $NameFilm = name_of_film;
                            ...
                            $('<div>', {class: 'united_el', style: 'max-width: 1200px; min-width: 300px; margin-left: auto; margin-right: auto'}).append(
                                $('<header>', {class: 'NameFilm_head'}).append(
                                    $('<h2>', {style: 'margin-bottom: 10px; display: inline-block; font: 20px normal tahoma, verdana, arial, sans-serif; color: #b5b5b5'}).append(
                                        ...
                                        GM.getValue('Alternative_NameFilm').then(alt_name => {
                    ...
                    $Video_pleer.appendTo($('div.united_el'));
                    GM.getValue('URL_Poster').then(url => {$Video_pleer.append($('<img>', {style: 'margin-left: 10px; height: 25%; width: 25%', class: 'poster', src: url}))});
                });
            }
        });
        ...
        let AdBanner = setInterval(() => {$('div[style="position: fixed !important; height: 305px !important; width: 480px !important; top: 0px !important; left: auto !important; z-index: 2147483647; right: 0px !important; margin: 0px !important;"]').remove()}, 50);
        setTimeout(() => {clearInterval(AdBanner)}, 1500);
        console.log('END.:.i;');
    };
