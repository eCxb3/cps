            ...
            let loading = function() {
                GM_xmlhttpRequest({
                    ...
                    onload: function(response) {
                        ...
                        if (response.finalUrl.includes('sso.passport.yandex.ru')) {
                            loading()
                            return;
                        }
                        ...
                        $Video_pleer.append($('<div>').append($('<img>', {style: 'margin-left: 10px; border-radius: 6px; box-shadow: 2px 2px 6px 3px rgba(0, 0, 0, 0.3);', class: 'poster', src: url, draggable: 'false'})));
    ...
    if (window.location.host === 'www.kinopoisk.ru') {
        document.addEventListener('DOMContentLoaded', function() {
            setInterval(function() {
                ...
                $('a[href]').filter(':not(.style_button__PNtXT):not(.processed):not(:contains("Смотреть"))').each(function() {
                    $(this).addClass('processed');
